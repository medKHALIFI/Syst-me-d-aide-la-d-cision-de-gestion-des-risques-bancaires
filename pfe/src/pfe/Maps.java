...
public class Maps {
    ...
    Node Winnig_node;
    double erreur_Moyenne=999999;///erreur moyenne
    int Cont_Iteration=0;
    double NeighbourhoodRadius=0;
    int dimMap;
    int Max_Iteration;
    double LEARNING_EREUR;
    double Start_Lerning_Rate;
    int Number_of_Node;
    ...
    //fonction de la creation de Map
   ...
   // System.out.println("donne le taux d'apprentissage initial");
    ...
    public void Normalisation(double[][]mat){
       ...
       return winner;
        ...
        public  double euclidean2(double[]x,double[][]poids,int nb_col,int numligne){//fonction qui permet de calculer la distance entre les neurones vanqueurs et les entrees des clients
   ...
   //fonction pour l'apprentisssage
    ...
    //fonction qui permet de calculer l'influence
    ...
    do{
      for(this_vector=0;this_vector<nb_ligne;this_vector++){
        for(int j=0;j<size_of_inputVector_and_Weight;j++) {input_vector[j]=matrix_input[this_vector][j];}
          ...
          erreur+=Winnig_node.euclidean( input_vector,size_of_inputVector_and_Weight);//la diference entre les poids et les entrées
...
*/
    ...
    erreur_Moyenne = erreur/nb_ligne;
           erreur=0;
           //Cont_Iteration < Max_Iteration  && erreur_Moyenne >LEARNING_EREUR 
    }while( (erreur_Moyenne > LEARNING_EREUR) && (Cont_Iteration < Max_Iteration));
       PrintWriter si=null;
       File f=new File("INFO.txt");
       
       si =new PrintWriter(new FileWriter(f));
       
       si.print(erreur_Moyenne);
       si.print("\t");
       si.print(Cont_Iteration);
       si.print("\t");
       si.print(Learning_Rate);
       si.println();
    ...
    //  for(int i=0;i<this_vector;i++){
     /*    
        d+=("l'entrée "+(i+1)+" neurone van  : ( "+ner[i].X_Node+","+ner[i].Y_Node+")");
          d+=("\n");
          v.t.setText(d);
       */   //v.t.setText("\n");
      ...
      */
      ...
      //  out.print("le nombre des entrées dans chaque vecteur:");out.print("\t");out.println(size_of_inputVector_and_Weight);
    ...
    //  out.println("l'entrée "+(i+1)+"--------->neurone van  : ( "+ner[i].X_Node+","+ner[i].Y_Node+" )");
